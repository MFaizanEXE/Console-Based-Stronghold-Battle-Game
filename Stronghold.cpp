...
int getValidInt(const string& prompt) {
    ...
    while (true) {
        ...
        try {
            ...
            cout << RED << "Invalid input. Enter a valid number.\n" << RESET;
        }
    }
    return input;
}
...
// General class
General::General(const string& name, double loyalty) : name(name), loyalty(loyalty), corrupted(false) {}
string General::getName() const { return name; }
bool General::isCorrupted() const { return corrupted; }
void General::setCorrupted(bool val) { corrupted = val; }

// King class
King::King(const string& name, double approval, const string& style)
...
void Population::adjustClassSize(const string& className, int delta) {
...
void Population::handleClassConflict() {
    if (morale < 0.4 && rand() % 10 < 3) {
        ...
        adjustMorale(-0.15);
        cout << RED << "Class conflict! Peasants and Merchants riot, population decreases.\n" << RESET;
    }
}
...
void Economy::collectTaxes(Population& pop) {
    ...
    cout << GREEN << "Collected " << tax << " gold in taxes.\n" << RESET;
}
...
void Economy::triggerMarketCrash(Population& pop) {
    if (rand() % 15 == 0) {
        ...
        cout << RED << "Market crash! Gold reserves drop by a third, morale plummets.\n" << RESET;
    }
}
...
void Blacksmith::upgrade(Economy& econ) {
    ...
    econ.spend(cost);
    level++;
    cout << GREEN << "Blacksmith upgraded to level " << level << "!\n" << RESET;
}
...
void Blacksmith::produceWeapons(Resource<int>& iron, Resource<int>& wood, int count) {
    ...
    wood.adjust(-count * 5);
    cout << "Producing " << count << " weapons...\n";
    this_thread::sleep_for(chrono::seconds(3 / level));
    weaponsInStock.adjust(count);
    cout << GREEN << "Produced " << count << " weapons!\n" << RESET;
}
...
Army::Army(int size, int weap) : soldiers(size), morale(0.8), weapons(weap), trainingDelay(0) {
    general = make_unique<General>("General Patton", 0.85);
}
...
void Army::train(int count, Population& pop, Resource<int>& iron, Blacksmith& blacksmith, double efficiency) {
    ...
    if (trainingDelay > 0) {
        cout << YELLOW << "Training delayed by " << trainingDelay << " turns.\n" << RESET;
        return;
    }
    ...
    cout << "Training " << count << " soldiers...\n";
    this_thread::sleep_for(chrono::seconds(static_cast<int>(5 * efficiency * (getGeneral().isCorrupted() ? 1.5 : 1.0))));
    soldiers += count;
    morale = (morale < 1.0) ? morale + 0.05 : 1.0;
    trainingDelay = getGeneral().isCorrupted() ? 2 : 1;
    cout << GREEN << "Trained " << count << " soldiers!\n" << RESET;
}
...
void Army::checkMorale(Economy& econ) {
    if (econ.getGold() < soldiers * 2) {
        morale -= 0.1;
        cout << RED << "Unpaid soldiers! Army morale drops.\n" << RESET;
    }
    if (morale < 0.3) {
        soldiers = (soldiers > soldiers / 10) ? soldiers - soldiers / 10 : 0;
        cout << RED << "Soldiers desert due to low morale!\n" << RESET;
    }
}
...
void Army::applyTrainingDelay() {
    if (trainingDelay > 0) {
        trainingDelay--;
        cout << YELLOW << "Training delay: " << trainingDelay << " turns remaining.\n" << RESET;
    }
}
...
// Politics class
Politics::Politics(const string& kingName) : currentKing(kingName), candidateCount(3), corrupted(false) {
    candidates[0] = make_unique<King>("Arthur", 0.8, "Diplomatic");
    candidates[1] = make_unique<King>("Eleanor", 0.75, "Economic");
    candidates[2] = make_unique<King>("Richard", 0.7, "Aggressive");
}

void Politics::holdElection(Population& pop, Economy& econ) {
    if (rand() % 10 == 0) {
        cout << RED << "Assassination! Current king killed, re-election triggered!\n" << RESET;
    ...
    if (corrupted) {
        currentKing = getCandidates()[rand() % getCandidateCount()]->getName();
        cout << RED << "Corrupt election! King chosen randomly.\n" << RESET;
        return;
    }
    ...
    pop.adjustMorale(0.05);
    cout << GREEN << "Election held! New king: " << currentKing << "\n" << RESET;
}

void Politics::bribe(Economy& econ, const string& candidate) {
    econ.spend(200);
    cout << YELLOW << "Bribed voters to favor " << candidate << ".\n" << RESET;
}

void Politics::blackmail(Economy& econ, const string& candidate) {
    econ.spend(300);
    cout << YELLOW << "Blackmailed voters to favor " << candidate << ", morale drops.\n" << RESET;
...
void Politics::triggerRebellion(Population& pop, Economy& econ) {
    if (pop.getMorale() < 0.3 && rand() % 5 == 0) {
        ...
        cout << RED << "Rebellion! Peasants revolt, treasury loses gold!\n" << RESET;
    }
}

unique_ptr<King>* Politics::getCandidates() { return candidates; }
int Politics::getCandidateCount() const { return candidateCount; }
string Politics::getCurrentKing() const { return currentKing; }
...
void Corruption::audit(Economy& econ, Army& army, Politics& politics, Blacksmith& blacksmith) {
    try {
        ...
        if (armyCorrupted) {
            ...
            cout << GREEN << "Army corruption cleared.\n" << RESET;
        }
        if (politicsCorrupted) {
            ...
            cout << GREEN << "Politics corruption cleared.\n" << RESET;
        }
        if (blacksmithCorrupted) {
            blacksmithCorrupted = false;
            blacksmith.setCorrupted(false);
            cout << GREEN << "Blacksmith corruption cleared.\n" << RESET;
        }
    }
    catch (const InsufficientResourcesException& e) {
        throw CorruptionException("Audit failed: " + string(e.what()));
    }
}
...
void Bank::takeLoan(Economy& econ, int amount) {
    ...
    cout << GREEN << "Took loan of " << amount << " gold.\n" << RESET;
}
...
void Bank::repayLoan(Economy& econ, int amount) {
    ...
    econ.spend(amount);
    loan -= amount;
    cout << GREEN << "Repaid " << amount << " gold.\n" << RESET;
}

void Bank::checkCorruption() {
    if (rand() % 20 == 0) {
        corrupted = true;
        cout << RED << "Bank corruption detected!\n" << RESET;
    }
}
...
void Bank::audit(Economy& econ) {
    try {
        econ.spend(100);
        corrupted = false;
        cout << GREEN << "Bank audit cleared corruption. Detailed report generated.\n" << RESET;
    }
    catch (const InsufficientResourcesException& e) {
        throw CorruptionException("Bank audit failed: " + string(e.what()));
    }
}
...
void Bank::seizeLand(Economy& econ, Map& map) {
    if (loan > 2000 && rand() % 5 == 0) {
        ...
        cout << RED << "Bank seized land due to unpaid loans!\n" << RESET;
    }
}
...
void Diplomacy::formAlliance(const string& kingdom) {
    if (allianceCount < 2) {
        alliances[allianceCount++] = { kingdom, true, false, false };
        cout << GREEN << "Alliance formed with " << kingdom << "!\n" << RESET;
    }
    else {
        cout << RED << "Cannot form more alliances.\n" << RESET;
    }
}

void Diplomacy::breakAlliance(const string& kingdom) {
    for (int i = 0; i < allianceCount; ++i) {
        if (alliances[i].kingdom == kingdom) {
            ...
            cout << YELLOW << "Alliance broken with " << kingdom << ".\n" << RESET;
            return;
        }
    }
    cout << RED << "No alliance with " << kingdom << ".\n" << RESET;
}

void Diplomacy::formTradeAgreement(const string& kingdom) {
    for (int i = 0; i < allianceCount; ++i) {
        if (alliances[i].kingdom == kingdom && alliances[i].active) {
            alliances[i].trade = true;
            cout << GREEN << "Trade agreement formed with " << kingdom << "!\n" << RESET;
            return;
        }
    }
...
void Diplomacy::establishSecureRoute(const string& kingdom) {
    for (int i = 0; i < allianceCount; ++i) {
        if (alliances[i].kingdom == kingdom && alliances[i].active) {
            alliances[i].secureRoute = true;
            cout << GREEN << "Secure trade route established with " << kingdom << "!\n" << RESET;
            return;
        }
    }
    throw InsufficientResourcesException("No alliance exists");
...
void Diplomacy::handleEspionageFailure(const string& sourceKingdom) {
    for (int i = 0; i < allianceCount; ++i) {
        if (alliances[i].kingdom == sourceKingdom) {
            alliances[i].active = false;
            alliances[i].trade = false;
            alliances[i].secureRoute = false;
            cout << RED << "Espionage detected! All alliances and trade agreements with "
...
bool Diplomacy::hasAlliance(const string& kingdom) const {
    for (int i = 0; i < allianceCount; ++i) {
...
bool Diplomacy::hasSecureRoute(const string& kingdom) const {
    for (int i = 0; i < allianceCount; ++i) {
...
void Communication::sendMessage(const string& recipient, const string& message, bool isFake) {
    if (messageCount < MAX_MESSAGES) {
        messages[messageCount++] = { recipient, message, isFake };
        cout << GREEN << "Message sent to " << recipient << ": " << message
            << (isFake ? " (fake)" : "") << "\n" << RESET;
    }
    else {
        cout << RED << "Message limit reached.\n" << RESET;
    }
}

void Communication::viewMessages(const string& kingdom) {
    cout << YELLOW << "Messages for " << kingdom << ":\n" << RESET;
    for (int i = 0; i < messageCount; ++i) {
        if (messages[i].recipient == kingdom) {
            cout << messages[i].content << (messages[i].isFake ? " (FAKE)" : "") << "\n";
        }
    }
}

void Communication::sendFakeTradeRequest(const string& recipient) {
...
void Healthcare::build(Economy& econ, Resource<int>& wood, Resource<int>& stone) {
    ...
    stone.adjust(-100);
    isBuilding = true;
    cout << "Building hospital...\n";
    this_thread::sleep_for(chrono::seconds(5));
    level++;
    satisfactionBoost += 0.02;
    plagueReduction += 0.05;
    isBuilding = false;
    cout << GREEN << "Hospital built! Level: " << level << RESET << "\n";
}

void Healthcare::provideServices(Population& pop) {
    pop.adjustMorale(satisfactionBoost);
    cout << GREEN << "Healthcare services boosted morale by " << satisfactionBoost << RESET << "\n";
}
...
void Buildings::buildBarracks(Economy& econ, Resource<int>& wood, Resource<int>& stone) {
    ...
    stone.adjust(-150);
    isBuilding = true;
    cout << "Building barracks...\n";
    this_thread::sleep_for(chrono::seconds(5));
    barracksLevel++;
    trainingEfficiency *= 0.9;
    isBuilding = false;
    cout << GREEN << "Barracks built! Level: " << barracksLevel << RESET << "\n";
}
...
void Weather::updateWeather() {
    ...
    cout << YELLOW << "Season: " << season << ", Weather: " << currentWeather << RESET << "\n";
}
...
string Weather::getSeason() const { return season; }
string Weather::getWeather() const { return currentWeather; }
...
void Inflation::update(Economy& econ, Bank& bank) {
    ...
    if (rate > 2.0) {
        cout << RED << "Bankruptcy! Gold devalued, morale drops.\n" << RESET;
...
void Map::capture(const string& kingdom, int x, int y) {
...
void Map::enemyAttack(Resource<int>& resource) {
    if (rand() % 10 < 3) {
        ...
        cout << RED << "Enemy attack! Lost " << loss << " resources.\n" << RESET;
    }
}
...
void Market::buyResource(Economy& econ, const string& resource, int amount, Resource<int>& res) {
    double cost = getPrice(resource) * amount;
    econ.spend(static_cast<int>(cost));
    res.adjust(amount);
    cout << GREEN << "Bought " << amount << " " << resource << " for " << cost << " gold.\n" << RESET;
}
...
void Market::handleSmuggler(Economy& econ, Resource<int>& resource) {
    if (smugglerActive) {
        resource.adjust(100);
        econ.spend(50);
        cout << GREEN << "Smugglers delivered 100 illegal goods for 50 gold.\n" << RESET;
    }
}
...
void Market::handleGuildDemands(Economy& econ, Population& pop) {
    if (guildDemands) {
        econ.spend(200);
        pop.adjustMorale(-0.05);
        cout << RED << "Trader guild demands met, cost 200 gold, morale drops.\n" << RESET;
    }
}
...
void Espionage::spyMission(Kingdom& source, Kingdom& target) {
    ...
    econ.spend(100);
    army.useSpies(5);
    cout << "Sending spies to " << target.getName() << "...\n";
    this_thread::sleep_for(chrono::seconds(3 + source.getWeather().getDelayImpact()));
    double successChance = 0.7 * (target.getPopulation().getMorale() < 0.5 ? 1.2 : 1.0);
    if (static_cast<double>(rand()) / RAND_MAX < successChance) {
        cout << GREEN << "Spy mission successful! Target status:\n" << RESET;
        target.printStatus();
        lastAction = "Spy Mission";
    }
    else {
        cout << RED << "Spy mission failed! Spies detected.\n" << RESET;
...
void Espionage::sabotageWeapons(Kingdom& source, Kingdom& target) {
    ...
    econ.spend(150);
    army.useSpies(10);
    cout << "Attempting to sabotage " << target.getName() << "'s weapons...\n";
    this_thread::sleep_for(chrono::seconds(4 + source.getWeather().getDelayImpact()));
    ...
    if (static_cast<double>(rand()) / RAND_MAX < successChance) {
        ...
        cout << GREEN << "Sabotage successful! Destroyed " << weaponsLost << " weapons.\n" << RESET;
        lastAction = "Sabotage Weapons";
    }
    else {
        cout << RED << "Sabotage failed! Spies detected.\n" << RESET;
        target.getDiplomacy().handleEspionageFailure(source.getName());
...
void Espionage::stealGold(Kingdom& source, Kingdom& target) {
    ...
    econ.spend(200);
    army.useSpies(15);
    cout << "Attempting to steal gold from " << target.getName() << "...\n";
    this_thread::sleep_for(chrono::seconds(5 + source.getWeather().getDelayImpact()));
    ...
    if (static_cast<double>(rand()) / RAND_MAX < successChance) {
        ...
        cout << GREEN << "Theft successful! Stole " << goldStolen << " gold.\n" << RESET;
        lastAction = "Steal Gold";
    }
    else {
        cout << RED << "Theft failed! Spies detected.\n" << RESET;
        target.getDiplomacy().handleEspionageFailure(source.getName());
...
void Smuggling::smuggleGoods(Kingdom& source, Kingdom& target) {
    ...
    econ.spend(100);
    cout << "Smuggling goods to " << target.getName() << "...\n";
    this_thread::sleep_for(chrono::seconds(3 + source.getWeather().getDelayImpact()));
    ...
    if (static_cast<double>(rand()) / RAND_MAX < successChance) {
        ...
        cout << GREEN << "Smuggling successful! Gained " << goods << " iron.\n" << RESET;
        lastAction = "Smuggle Goods";
    }
    else {
        cout << RED << "Smuggling failed! Goods seized.\n" << RESET;
        econ.spend(50);
...
// Kingdom class
Kingdom::Kingdom(const string& kingdomName, const string& kingName)
    : name(kingdomName), food(1000), iron(500), wood(800), stone(600) {
    population = make_unique<Population>();
    economy = make_unique<Economy>(1000);
    army = make_unique<Army>(100, 100);
    bank = make_unique<Bank>();
    politics = make_unique<Politics>(kingName);
    blacksmith = make_unique<Blacksmith>();
    diplomacy = make_unique<Diplomacy>();
    communication = make_unique<Communication>();
    healthcare = make_unique<Healthcare>();
    buildings = make_unique<Buildings>();
    weather = make_unique<Weather>();
    inflation = make_unique<Inflation>();
    corruption = make_unique<Corruption>();
    map = make_unique<Map>();
    market = make_unique<Market>(inflation.get());
}

void Kingdom::playTurn() {
    cout << BOLD << "=== Turn in " << name << " ===\n" << RESET;
    weather->updateWeather();
    food.adjust(weather->getFoodImpact());
    if (weather->getFoodImpact() < 0)
        cout << RED << "Weather reduced food by " << -weather->getFoodImpact() << "!\n" << RESET;
    else if (weather->getFoodImpact() > 0)
        cout << GREEN << "Weather increased food by " << weather->getFoodImpact() << "!\n" << RESET;
...
void Kingdom::randomEvent() {
    ...
    case 0:
        ...
        cout << RED << "Plague spreads! Population decreases significantly.\n" << RESET;
        break;
    case 1:
        economy->spend(economy->getGold() / 10);
        cout << RED << "Bandits raid the treasury!\n" << RESET;
        break;
    case 2:
        food.adjust(500);
        population->adjustMorale(0.1);
        cout << GREEN << "Bumper harvest! Food increases.\n" << RESET;
        break;
    case 3:
        food.adjust(-300);
        cout << RED << "Drought! Food supply decreases.\n" << RESET;
        break;
    case 4:
        market->updatePrices();
        cout << RED << "Sanctions imposed! Market prices increase.\n" << RESET;
        break;
    case 5:
        population->adjustMorale(-0.1);
        politics->getCandidates()[rand() % politics->getCandidateCount()]->setCorrupted(true);
        cout << RED << "Assassination attempt on king! Candidate corrupted.\n" << RESET;
        break;
    case 6:
        economy->triggerMarketCrash(*population);
        cout << RED << "Market crash! Prices soar.\n" << RESET;
        break;
    case 7:
        population->adjustMorale(-0.05);
        cout << RED << "Revolt risk rises!\n" << RESET;
        break;
    case 8:
        population->adjustClassSize("Nobility", -population->getClasses()[2].size / 2);
        population->adjustMorale(-0.1);
        cout << RED << "Noble uprising! Nobility population halved.\n" << RESET;
        break;
    case 9:
        army->getGeneral().setCorrupted(true);
        cout << RED << "Spy infiltration! General corrupted.\n" << RESET;
        break;
    }
}
...
void Kingdom::buyResource(const string& resource, int amount) {
...
void Kingdom::manageDiplomacy(const string& kingdom, int choice) {
...
void Kingdom::bribeOrBlackmail(int choice, const string& candidate) {
    if (choice == 1) politics->bribe(*economy, candidate);
    else if (choice == 2) politics->blackmail(*economy, candidate);
}

void Kingdom::sendMessage(const string& recipient, const string& message) {
    communication->sendMessage(recipient, message, false);
}

void Kingdom::sendFakeTradeRequest(const string& recipient) {
...
void Kingdom::saveState(const string& filename) const {
    ofstream file(filename, ios::app);
    if (!file.is_open()) throw runtime_error("Cannot open save file");
    file << "Kingdom: " << name << "\n";
    ...
    file << "\n";
    file.close();
    cout << GREEN << "Game state saved for " << name << "!\n" << RESET;
}

void Kingdom::loadState(const string& filename) {
    ifstream file(filename);
    if (!file.is_open()) {
        cout << YELLOW << "No save file found for " << name << ". Starting new game.\n" << RESET;
        return;
    }
    string line, savedName;
    while (getline(file, line)) {
        if (line.find("Kingdom: ") == 0) {
            savedName = line.substr(9);
            if (savedName != name) continue;
            cout << GREEN << "Game state loaded for " << name << "!\n" << RESET;
            break;
        }
    }
    file.close();
...
void Kingdom::saveScore() const {
    ...
    if (!file.is_open()) throw runtime_error("Cannot open score.txt");
    time_t now = time(nullptr);
#ifdef _MSC_VER
    char buffer[26];
    if (ctime_s(buffer, sizeof(buffer), &now) != 0) {
        throw runtime_error("Failed to format timestamp");
    }
#else
    ...
    file.close();
    cout << GREEN << "Score saved to score.txt for " << name << "!\n" << RESET;
}

int Kingdom::calculateScore() const {
    int moraleScore = static_cast<int>(population->getMorale() * 300);
    int goldScore = min(1000, economy->getGold() / 10) * 250;
...
void Kingdom::printStatus() const {
    cout << YELLOW << "Kingdom Status (" << name << "):\n" << RESET;
    cout << "Population: " << population->getTotalSize() << ", Morale: " << population->getMorale() << "\n";
    ResourcePair* classes = population->getClasses();
    for (int i = 0; i < MAX_CLASSES; ++i) {
        cout << "  " << classes[i].name << ": " << classes[i].size << ", Satisfaction: " << classes[i].satisfaction << "\n";
    }
    cout << "Gold: " << economy->getGold() << ", Loan: " << bank->getLoan() << ", Debt Reliance: " << economy->getDebtReliance() << "\n";
    cout << "Army: " << army->getSize() << ", Morale: " << army->getMorale() << ", Weapons: " << army->getWeapons() << "\n";
    cout << "Resources: Food=" << food.get() << ", Iron=" << iron.get()
        << ", Wood=" << wood.get() << ", Stone=" << stone.get() << "\n";
    cout << "Blacksmith: Level=" << blacksmith->getLevel() << ", Weapons in stock=" << blacksmith->getWeaponsInStock() << "\n";
    cout << "Healthcare: Level=" << healthcare->getLevel() << ", Plague Reduction=" << healthcare->getPlagueReduction() * 100 << "%\n";
    cout << "Barracks: Level=" << buildings->getBarracksLevel() << ", Training Efficiency="
        << buildings->getTrainingEfficiency() * 100 << "%\n";
    cout << "Weather: " << weather->getSeason() << ", " << weather->getWeather() << "\n";
    cout << "Inflation: " << inflation->getRate() << "\n";
    cout << "King: " << politics->getCurrentKing() << "\n";
    cout << "Tax: " << (economy->isProgressiveTax() ? "Progressive" : "Flat") << "\n";
    cout << "Land Seized by Bank: " << bank->getLandSeized() << "\n";
    cout << "Score: " << calculateScore() << " points\n";
    map->display();
}
